...
class IllegalFlyingGoal(val entity: MagicBookEntity) : Goal() {
    ...
    override fun tick() {
        ...
        if (time > interval && illegal) {
            ...
            target.abilities.mayfly = false
            // 关闭飞行符文

            target.inventory.items.forEach {
                if (it.isOf(UsefulMagicItems.FLYING_RUNE)) {
                    it.set(UsefulMagicDataComponentTypes.ENABLED.get(), false)
                }
            }
