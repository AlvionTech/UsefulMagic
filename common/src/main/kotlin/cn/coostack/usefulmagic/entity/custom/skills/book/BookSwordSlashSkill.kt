 ...
 */
class BookSwordSlashSkill(val damage: Float) : Skill<MagicBookEntity>, SkillCondition<MagicBookEntity> {
    ...
    override fun holdingTick(holdingEntity: MagicBookEntity, holdTicks: Int) {
        ...
        if (holdTicks % 4 == 0) {
            repeat(6) {
                ...
                val pos = holdingEntity.position().add(x, y, z)
                val line = RelativeLocation(0.0, random.nextDouble(1.0, 5.0), 0.0)
                val count = (line.length() * 2).roundToInt()
                val style = EnchantLineComposition(pos, holdingEntity.level()).apply {
                    end = line
                    this.count = count
                    lifetime = random.nextInt(30, 45)
                }
                style.apply {
        ...
        holdingEmitters?.teleportTo(holdingEntity.position())
    ...
    private fun handleOnceDamage(attacker: MagicBookEntity) {
